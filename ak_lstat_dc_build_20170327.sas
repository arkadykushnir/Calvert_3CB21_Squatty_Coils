...
Create CALVERT.ak_lstat_dc_build_20170327 Table
...
PROC MEANS DATA=dc_mpoints MAXDEC=1 NOPRINT NWAY MEAN MIN MAX STD;
  BY id_piece;
  ID dc_used;
 ...
 CLASS coil_location;
  ...
  ID dc_used;
    VAR force_sg force_sg_os force_sg_ds gap_sg force_pr force_pr_os 
		force_pr_ds diff_force_pr level_pr speed_top_pr speed_bot_pr tens_mandrel torque_mandrel 
		expan_mandrel speed_mandrel force_wr1 force_wr2 force_wr3 pos_wr1 pos_wr2 pos_wr3;
   OUTPUT OUT=dc_coilloc (DROP=_TYPE_ _FREQ_)
;
run;
...
DATA ak_lstat_dc;
 SET dc_coillocO (IN=clocO)
     dc_coilloc (IN=cloc);
run;


PROC SORT DATA=ak_lstat_dc;
 BY id_piece coil_location _STAT_;
run;
...
DATA dblib.ak_lstat_dc (BULKLOAD=YES 
...
/* *** EXPORT sak_lstat_dc into .csv for Paul (to Run PYTON Analytics) *** */
FILENAME csv_out "C:\Users\ak186133\Remote Calvert ASTER Project\SAS_Outputs\ak_lstat_dc_&SYSDATE9..csv";
PROC EXPORT DATA=ak_lstat_dc
            OUTFILE=csv_out
  DBMS=csv REPLACE;
run;
...
/* *** 508 (Squatty Coils Rejects *** */
           LEFT OUTER JOIN dedup_rejects_508 rej
		    on rej.coil_number = INPUT(base.pc_id_piece,10.)
...
DATA my_sas.coils_3cb2021_20170324;
 SET steel_3cb2021;
run;
